...
const buildGradients = function (saturation) {
	...
	for (let i = 0; i < numSlices; i++) {
		...
		ret += '\n<stop offset="0%" stop-color="hsl(' + i + ', ' + saturation + '%, 100%)"></stop>';
		ret += '\n<stop offset="50%" stop-color="hsl(' + i + ', ' + saturation + '%, 50%)"></stop>';
		ret += '\n<stop offset="100%" stop-color="hsl(' + i + ', ' + saturation + '%, 0%)"></stop>';
		ret += '\n</radialGradient>';
	}
	return ret;
};
