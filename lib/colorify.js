...
const getCombinationFunctionNames = function() {
  return Object.keys(combinationFunctions);
};

const getCssFormatTypes = function() {
  return ['hex', 'rgb', 'percent', 'hsl', 'hwb'];
};

/* convert a onecolor object to a css string */
const format = function(oneColorObject, formatType) {
  let type = 'hex';
...
const isBitOn = function(num, pos) {
  return (num & Math.pow(2, pos)) >> pos === 1;
};

const combineColors = function(color1, color2, colorSpace, flags, fn) {
  let i;
  let key;
  let fnName;
  let val;
